...
# Blockchain part for the transaction
class Block:
...
if __name__ == "__main__":
    # List of items
    items = [
        "Sofa", "Television", "Bed", "Toaster", "Coffee Maker",
        "T-Shirt", "Laptop", "Dining Table", "Refrigerator", "Chair",
        "Blender", "Jeans", "Smartphone", "Nightstand", "Microwave",
        "Shirt", "Tablet", "Desk Lamp", "Vacuum Cleaner", "Couch"
    ]
    ...
    # Mapping of store names to Store objects
    store_mapping = {
        "Store_A": store_a,
        "Store_B": store_b,
        "Store_C": store_c
    ...
    # Rank and display orders of the stores
    ranked_store = []
    for rank, (store_name, _) in enumerate(ranked_stores, start=1):
        # Get the store object from the mapping
        store_obj = store_mapping.get(store_name)
        
        if store_obj:  # Ensure store_obj exists
            print(f"{rank}. {store_name} Orders: {store_obj.orders}")
            print("\n")
            ranked_store.append(f"{rank}. {store_name} Orders: {store_obj.orders}")
        else:
            print(f"{rank}. {store_name}: No store object found")
